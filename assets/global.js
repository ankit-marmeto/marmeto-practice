...
const flickityContainer = document.querySelectorAll('.section-brand-collections__continer');
flickityContainer.forEach((element) => {
  if (viewportWidth < 750) {
    element.setAttribute('data-flickity', JSON.stringify(flickityOptions));
  } else {
    // Remove the data-flickity attribute to destroy Flickity
...
//    when user click on filter options or sort options, aria-selected attribute is added to body
      if (event.target.matches('.filter')) {
        document.querySelector('body').setAttribute('aria-selected', 'filter-options');
      }
      if(event.target.matches('.sort')) {
          document.querySelector('body').setAttribute('aria-selected', 'sort-options');
      }
    ...
    const announcementBar = document.querySelector('.main_announcement_bar').parentNode;
...
class SliderComponent extends HTMLElement {
  ...
  onButtonClick(event) {
    ...
    this.slideScrollPosition =
      event.currentTarget.name === 'next'? this.slider.scrollLeft + step * this.sliderItemOffset: this.slider.scrollLeft - step * this.sliderItemOffset;
...
class VariantSelects extends HTMLElement {
  ...
  /*
   * for color swatches the data attribute for this (custom-element) is updated
  */
  updateURL() {
    if (!this.currentVariant || this.dataset.updateUrl === 'false') return;
    if (event.target.name === 'color' || event.target.name === 'colour') window.location.href = event.target.dataset.url;
    else window.history.replaceState({}, '', `${this.dataset.url}?variant=${this.currentVariant.id}`);
  }
  ...
  /* 
   * if option is color, it acts as a different product, fetch that product object - variants object in product-variant-options.liquid
   * if option is size, it stays in same product, so fetch the existing product object - variants object in product-variant-picker.liquid
  */
  getVariantData() {
    if ( event.target.name == 'color' || event.target.name == 'colour' ) {
      const colorSwatchProduct = event.target.nextElementSibling.nextElementSibling;
      this.variantData = JSON.parse(colorSwatchProduct.textContent);
    } else {
      this.variantData = this.variantData || JSON.parse(this.querySelector('#product_variants_JSON[type="application/json"]').textContent);
    }
      ...
      fetch(this.dataset.url)
        ...
        .then((text) => {
          ...
          if (recommendations && recommendations.innerHTML.trim().length) {
            ...
            const wishlistIconArray = document.querySelector("product-recommendations").querySelectorAll(".wishlist-container")
...
let grayscaleToggle = false;

function startToggling() {
  const firstImage = document.querySelector('.banner .first-image img');
  const secondImage = document.querySelector('.banner .second-image img');

  if(firstImage && secondImage){
     setInterval(() => {
      if (window.innerWidth < 750) {
        if (grayscaleToggle) {
          secondImage.style.filter = 'grayscale(100%)';
        } else {
          secondImage.style.filter = 'none';
        }
      } else {
        if (grayscaleToggle) {
          firstImage.style.filter = 'none';
          secondImage.style.filter = 'grayscale(100%)';
        } else {
          firstImage.style.filter = 'grayscale(100%)';
          secondImage.style.filter = 'none';
        }
      }
    ...
    const intersectionObserver = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          startToggling();
          intersectionObserver.unobserve(bannerSection); // Stopping the observer once the section is in viewport
        }
      });
    });
    
    intersectionObserver.observe(bannerSection);
  }
} else {
  startToggling();
}

const rttContainer = document.querySelector('.rtt_container');
if(rttContainer){
  window.addEventListener('scroll', function() {
    const scrollPosition = window.scrollY || document.documentElement.scrollTop;
    // Check if the user has scrolled beyond the first fold
    if (scrollPosition > window.innerHeight) {
      rttContainer.classList.add('show');
    } else {
      rttContainer.classList.remove('show');
    }
  });
} 

document.addEventListener("DOMContentLoaded", function() {
  document.querySelectorAll('#offer-splide, #offer-splide-drawer').forEach(sliderElement => {
...
//announcementbar splide JS 
document.addEventListener("DOMContentLoaded", function() {
  const slider = new Splide('#announcement-splide', {
      type: 'slide',
      perPage: 1,
      perMove: 1,
      focus: 'left',        
