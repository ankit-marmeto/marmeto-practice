...
// Custom splide component

class ComponentSplide extends HTMLElement {
  constructor() {
    super();
    this.splideContainer = this.querySelector(".splide");
    this.splideData = this.getAttribute("data-splide");
    this.parsedData = JSON.parse(this.splideData);
    this.initCarousal();
  }

  initCarousal() {
    this.splide = new Splide(this.splideContainer, this.parsedData).mount();
    // Add custom colors to pagination dots after mount
    const dots = document.querySelectorAll(".custom-dot");
    const colors = ["#FFD700", "#32CD32", "#FF4500"];
...
customElements.define("component-splide", ComponentSplide);
...
class CustomProductForm extends HTMLElement{
  ...
  onSubmitHandler(evt) {
    evt.preventDefault();
    debugger
    this.secondCard = this.querySelector("[data-product-id]").dataset.productId;
    ...
    formData.append(`items[1][id]`, this.secondCard);
...
class MarmetoProductCard extends HTMLElement {
  constructor() {
    ...
    this.selectedVariant = this.submitButton.dataset.variantId;
    ...
    fetch("/cart/add.js", {
      ...
      .then((response) => {
        if (response.status) {
          ...
          // Open AJAX cart or redirect to cart
...
customElements.define("custom-product-card", MarmetoProductCard);